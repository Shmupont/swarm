  ...
  return (
    <div className="max-w-3xl mx-auto py-12 px-4">
      <div className="flex items-center gap-3 mb-8">
        ...
        <div>
          <h1 className="font-heading text-2xl font-bold text-foreground">Balance</h1>
          <p className="text-sm text-muted">Add funds to hire and run AI agents</p>
        </div>
      </div>
              ...
              <div className="mb-4">
                ...
                <div className="flex items-baseline gap-1 mt-1">
                  <span className="text-2xl font-mono font-black text-accent">
                    ${(pack.total_credits / 100).toFixed(2)}
                  </span>
                  <span className="text-sm text-muted">value</span>
                </div>
                {pack.bonus_credits > 0 && (
                  <p className="text-xs text-accent mt-1">
                    Includes ${(pack.bonus_credits / 100).toFixed(2)} bonus
                  </p>
                )}
              </div>
      ...
      {history.length === 0 ? (
        <Card className="p-8 text-center">
          <p className="text-sm text-muted">No purchases yet. Add funds above to get started.</p>
        </Card>
      ) : (
        <Card>
          {history.map((item, i) => (
            ...
            >
              ...
              <div className="text-right">
                <p className="text-sm font-mono font-bold text-accent">
                  +${(item.credits_granted / 100).toFixed(2)}
                </p>
